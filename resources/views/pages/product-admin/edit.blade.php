...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-sm p-8">
        ...
        <h3 class="text-lg font-bold text-gray-800 mb-6">Edit Produk</h3>
        ...
        <form action="{{ route('product-admin.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
                ...
                {{-- Input: Estimasi Pengerjaan — nilai diisi dari $product->production_time --}}
                <div>
                    ...
                    <input type="number" name="production_estimate" placeholder="Masukan estimasi pengerjaan"
                        value="{{ old('production_estimate', $product->production_estimate) }}"
                        class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none focus:border-[#8A9E5B] focus:ring-1 focus:ring-[#8A9E5B] transition placeholder:text-gray-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <textarea name="description" rows="10" placeholder="Tuliskan keterangan kue"
                        class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none focus:border-[#8A9E5B] focus:ring-1 focus:ring-[#8A9E5B] transition placeholder:text-gray-300 resize-y">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block text-sm text-gray-600 mb-1.5">
                        Foto produk <span class="text-red-500">*</span> {{-- Tanda * = field wajib --}}
                    </label>
